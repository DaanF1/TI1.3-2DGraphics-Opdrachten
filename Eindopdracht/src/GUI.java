...
public class GUI extends Application{
    ...
    private void update(double deltaTime) {
        // Eigen gemaakte formules om alle objecten rond (elkaar) te laten draaien
        ...
        for (Planet planet : solarSystem.getPlanets()){
            ...
            // Als de planeet een Scattered Disk Object is, moet de baan ovaal worden
            if (planet.isAScatteredDiskObject()){
                ...
                if (planet.getAngle()%planet.getFullRotation() < -Math.PI){
                    planet.setDistanceToSun((int) (planet.getDistanceToSun()+distanceMultiplier));
                } else if (planet.getAngle()%planet.getFullRotation() > -Math.PI){
                    planet.setDistanceToSun((int) (planet.getDistanceToSun()-distanceMultiplier));
                } else if (planet.getAngle()%planet.getFullRotation() == 2*Math.PI){
                    planet.setFullRotation(planet.getFullRotation() + (float) (2*Math.PI));
                }
            }
        }
    }
